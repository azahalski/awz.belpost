...
use Awz\Belpost\Access\AccessController;

Loc::loadMessages(__FILE__);
global $APPLICATION;
$module_id = "awz.belpost";
if(!Loader::includeModule($module_id)) return;
Extension::load('ui.sidepanel-content');
$request = Application::getInstance()->getContext()->getRequest();
$APPLICATION->SetTitle(Loc::getMessage('AWZ_BELPOST_OPT_TITLE'));
...
$deliveryProfileList = \Awz\Belpost\Helper::getActiveProfileIds();
...
if ($request->getRequestMethod()==='POST' && AccessController::isEditSettings() && $request->get('Update'))
{
    ...
    if($_REQUEST['DELETE_PVZ']=='Y'){
        \Awz\Belpost\PvzTable::deleteAll();
        CAdminMessage::ShowMessage(array('TYPE'=>'OK',
            'MESSAGE'=>Loc::getMessage('AWZ_BELPOST_OPT_MESS1')));
        $sendRunAgent = true;
    }
    if($_REQUEST['UPDATE_PVZ']=='Y'){
        $strAgent = \Awz\Belpost\Checker::agentGetPickpointsStep(0, time()+20);
        if($strAgent){
            \CAgent::AddAgent(
                    $strAgent,
                    "awz.belpost",
                    ...
                    60);
            CAdminMessage::ShowMessage(array('TYPE'=>'OK',
                    'MESSAGE'=>Loc::getMessage('AWZ_BELPOST_OPT_MESS2')));
        }else{
            CAdminMessage::ShowMessage(array('TYPE'=>'OK',
                    'MESSAGE'=>Loc::getMessage('AWZ_BELPOST_OPT_MESS2_ONE')));
        }
    ...
    $townsPrep = [];
    try{
        $townsPrep = unserialize(Option::get('awz.belpost', "REPL_TOWNS", "",""), ['allowed_classes' => false]);
    }catch (\Exception $e){
        $townsPrep = [];
    }
...
$aTabs[] = array(
    "DIV" => "edit1",
    "TAB" => Loc::getMessage('AWZ_BELPOST_OPT_SECT1'),
    "ICON" => "vote_settings",
    "TITLE" => Loc::getMessage('AWZ_BELPOST_OPT_SECT1')
);
...
foreach($deliveryProfileList as $profileId=>$profileName){
    ...
    $aTabs[] = array(
        "DIV" => "edit".$cnt,
        "TAB" => Loc::getMessage('AWZ_BELPOST_OPT_SECT2',array('#PROFILE_NAME#'=>'['.$profileId.'] - '.$profileName)),
        "ICON" => "vote_settings",
        "TITLE" => Loc::getMessage('AWZ_BELPOST_OPT_SECT2',array('#PROFILE_NAME#'=>'['.$profileId.'] - '.$profileName))
    );
}
$cnt++;
...
<tr>
    <td width="50%"><?=Loc::getMessage('AWZ_BELPOST_OPT_MAP_ADRESS')?><br>
        <a href="https://developer.tech.yandex.ru/services" target="_blank">
            <?=Loc::getMessage('AWZ_BELPOST_OPT_MAP_ADRESS_DESC')?>
        </a>
    </td>
    <td>
...
<tr>
    <td width="50%"><?=Loc::getMessage('AWZ_BELPOST_OPT_MAP_ADRESS_KEY1')?></td>
    <td>
    ...
    <tr>
    <td width="50%"><?=Loc::getMessage('AWZ_BELPOST_OPT_MAP_ADRESS_KEY2')?></td>
    <td>
...
<tr class="heading">
    <td colspan="2">
        <?=Loc::getMessage('AWZ_BELPOST_OPT_L_GROUP1')?>
    </td>
</tr>
<tr>
    <td colspan="2" align="center">
        <div class="adm-info-message-wrap">
            <div class="adm-info-message">
                <div> <?=Loc::getMessage('AWZ_BELPOST_OPT_L_GROUP1_DESC')?></div>
            </div>
        </div>
    </td>
</tr>
<tr>
    <td width="50%"><?=Loc::getMessage('AWZ_BELPOST_OPT_L_DELPVZ')?></td>
    <td>
        <?$val = "N";?>
        <input type="checkbox" value="Y" name="DELETE_PVZ" <?if ($val=="Y") echo "checked";?>></td>
</tr>
<tr>
    <td width="50%"><?=Loc::getMessage('AWZ_BELPOST_OPT_L_UPPVZ')?></td>
    <td>
        <?$val = "N";?>
    ...
    <tr class="heading">
        <td colspan="2">
            <?=Loc::getMessage('AWZ_BELPOST_OPT_L_SOOT')?></td>
    </tr>
<tr>
    <td colspan="2" width="100%">
        ...
        $allTowns = \Awz\Belpost\PvzTable::getList([
            'select'=>['TOWN'],
            'group'=>['TOWN']
        ]);
        ?>
        <table style="margin:auto;">
            <tr>
                <th style="text-align: left;"><?=Loc::getMessage('AWZ_BELPOST_OPT_L_SOOT_LBL1')?></th>
                <th style="text-align: left;"><?=Loc::getMessage('AWZ_BELPOST_OPT_L_SOOT_LBL2')?></th>
            </tr>
        <?
        $keyIssets = [];
...
<?foreach($deliveryProfileList as $profileId=>$profileName){
    ...
    $pvzList = \Awz\Belpost\Helper::getActiveProfileIds(\Awz\Belpost\Helper::DOST_TYPE_PVZ);
    ...
    ?>
        ...
        <tr class="heading">
            <td colspan="2">
                <?=Loc::getMessage('AWZ_BELPOST_OPT_L_PROFILE_PROP')?>
            </td>
        </tr>

        <?if($isPvz){?>
            <tr>
                <td><?=Loc::getMessage('AWZ_BELPOST_OPT_L_PROPPVZ')?></td>
                <td>
                    <?$val = Option::get($module_id, "PVZ_CODE_".$profileId, "AWZ_BP_POINT_ID", "");?>
                    <input type="text" size="35" maxlength="255" value="<?=$val?>" name="PVZ_CODE_<?=$profileId?>"/>
                </td>
            </tr>

            <tr>
                <td><?=Loc::getMessage('AWZ_BELPOST_OPT_L_PROPPVZ_ADR')?></td>
                <td>
        ...
        <tr>
            <td width="50%"><?=Loc::getMessage('AWZ_BELPOST_OPT_L_PROPPVZ_SHOW_ALL')?>
            </td>
            <td>
...
$tabControl->Buttons();
?>
<input <?if (!AccessController::isEditSettings()) echo "disabled" ?> type="submit" class="adm-btn-green" name="Update" value="<?=Loc::getMessage('AWZ_BELPOST_OPT_L_BTN_SAVE')?>" />
...
<input type="hidden" name="code" value="<?=htmlspecialcharsEx($startCode)?>">
    <?if(AccessController::isViewRight()){?>
        <button class="adm-header-btn adm-security-btn" onclick="BX.SidePanel.Instance.open('<?=$saveUrl?>');return false;">
            <?=Loc::getMessage('AWZ_BELPOST_OPT_SECT3')?>
        </button>
    <?}?>
<?$tabControl->End();?>
</form>
