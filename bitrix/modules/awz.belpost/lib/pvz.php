...
class PvzTable extends Entity\DataManager
{
    ...
    public static function getMap()
    {
        return array(
            new Entity\IntegerField('ID', array(
                'primary' => true,
                'autocomplete' => false,
                'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_ID')
                )
            ),
            new Entity\StringField('PVZ_ID', array(
              'required' => true,
              'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_PVZ_ID')
                )
            ),
            new Entity\StringField('TOWN', array(
              'required' => true,
              'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_TOWN')
                )
            ),
            new Entity\StringField('CITY_TYPE', array(
                    'required' => true,
                    'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_CITY_TYPE')
                )
            ),
            new Entity\StringField('DISTRICT', array(
                    'required' => true,
                    'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_DISTRICT')
                )
            ),
            new Entity\StringField('REGION', array(
                    'required' => true,
                    'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_REGION')
                )
            ),
            new Entity\StringField('PRM', array(
                'required' => false,
                'serialized'=>true,
                'title'=>Loc::getMessage('AWZ_BELPOST_PVZ_FIELDS_PRM')
                )
            ),
        );
    }
    ...
    public static function checkPvzFromTown($town){
        ...
        if(!$res->fetch()){
            $result->addError(
                new Error(
                    Loc::getMessage('AWZ_BELPOST_PVZ_ERR_TOWN',
    ...
    public static function updatePvz($data){
        if($data['id']){
            ...
            $towns = [];
            try{
                $towns = unserialize(Option::get('awz.belpost', "REPL_TOWNS", "",""), ['allowed_classes' => false]);
            }catch (\Exception $e){
                $towns = [];
            }
