...
class handlersBx {
    ...
    public static function registerHandler()
    {
        ...
        $result = new \Bitrix\Main\EventResult(
            ...
            array(
                'Awz\Belpost\Handler' => '/bitrix/modules/awz.belpost/lib/handler.php',
                'Awz\Belpost\Profiles\Pickup' => '/bitrix/modules/awz.belpost/lib/profiles/pickup.php',
                //'Awz\Belpost\Profiles\Standart' => '/bitrix/modules/awz.belpost/lib/profiles/standart.php',
            )
        );
    ...
    public static function OnAdminSaleOrderEditDraggable($args){
        $res = array(
            'getScripts'=>array('\Awz\Belpost\handlersBx','editDraggableAddScript')
        );
        return $res;
    }
        ...
        if(isset($args['ORDER']) && $args['ORDER'] instanceof Order){
            ...
            $content .= "BX.insertAfter(BX.create('a', {
                      attrs: {
                         ...
                         href: '/',
                         onclick: 'BX.SidePanel.Instance.open(\"/bitrix/admin/awz_belpost_picpoint_list.php?LANG=ru&page=order_edit&order=".$order->getId()."\",{cacheable: false});return false;'
                      },
                      text: '".Loc::getMessage('AWZ_BELPOST_HANDLERBX_CHOISE')."'
    ...
    public static function OnEndBufferContent(&$content){
        ...
        if($APPLICATION->getCurPage(false) == '/bitrix/admin/sale_order_ajax.php'){
            if($_REQUEST['action'] == 'changeDeliveryService' && $_REQUEST['formData']['order_id']){
                if(!\Bitrix\Main\Loader::includeModule('awz.belpost')) return;
                ...
                if($profileId){
                    $delivery = \Awz\Belpost\Helper::deliveryGetByProfileId($profileId);
                    //print_r($delivery);die();
                    if(in_array($delivery['CLASS_NAME'],array('\Awz\Belpost\Profiles\Pickup', '\Awz\Belpost\Handler'))){
                        $json = \Bitrix\Main\Web\Json::decode($content);
                        if($delivery['CLASS_NAME'] == '\Awz\Belpost\Handler'){
                            preg_match('/value="([0-9]+)"/is',$json['SHIPMENT_DATA']['PROFILES'], $mc);
                            $profileId = $mc[1];
                        }
                        $json['SHIPMENT_DATA']['PROFILES'] .= '<br><a href="#" class="adm-btn adm-btn-green adm-btn-add" onclick="BX.SidePanel.Instance.open(\'/bitrix/admin/awz_belpost_picpoint_list.php?LANG=ru&profile_id='.intval($profileId).'&order='.intval($_REQUEST['formData']['order_id']).'&from=changeDeliveryService\',{cacheable: false});return false;">'.Loc::getMessage('AWZ_BELPOST_HANDLERBX_CHOISE_PVZ').'</a>';
        ...
        if(!$checkMyDeliveryPvz) {
            ...
            if($pointId){
                ...
                if($pointData){
                    ...
                    //$setPoints = false;
                    $errorText = Loc::getMessage('AWZ_BELPOST_HANDLERBX_ERR_PVZDATA');
                }
            }

            if(!$setPoints || $errorText){
                if(!$errorText) $errorText = Loc::getMessage('AWZ_BELPOST_HANDLERBX_ERR_PVZ');
                if(!$checkIsProp){
                    $errorText = Loc::getMessage('AWZ_BELPOST_HANDLERBX_ERR_PVZ_PROP');
                }
                $event->addResult(
                    new \Bitrix\Main\EventResult(
    ...
    public static function OrderDeliveryBuildList(&$arResult, &$arUserResult, $arParams)
    {
        ...
        Asset::getInstance()->addString('<script>window._awz_bp_lib_setSearchAddress = "'.$setSearchAddress.'";</script>', true);
