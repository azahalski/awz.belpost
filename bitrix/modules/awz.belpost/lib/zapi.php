...
class zApi {
     ...
     */
    const URL = 'https://dostavka.zahalski.dev/api/belpost';

    /**
     * Папка для кеша в /bitrix/cache/
     */
    const CACHE_DIR = '/awz/belpost/';
    ...
    public function getDefTarifs(string $type = ''){
        $url = 'tarifs?type='.$type;
        return $this->send($url, [], 'get');
    }

    public function getPvz($data = array()){
        $url = 'pickpoints?type=all';
        if(isset($data['next_page']) && $data['next_page']){
            $url .= '&next_page='.$data['next_page'];
        }
    ...
    protected function send($method, $data = array(), $type='post'){
        ...
        if(!$res){
            $result->addError(
                new Error(Loc::getMessage('AWZ_BELPOST_YDAPI_RESPERROR'))
            );
        }else{
            try {
