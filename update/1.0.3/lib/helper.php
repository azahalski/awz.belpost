...
class Helper {
    ...
    public static function deliveryGetByProfileId($profileId){
        if(!Loader::includeModule('sale')){
            throw new SystemException(
                Loc::getMessage('AWZ_BELPOST_HELPER_NOT_SALE_MODULE')
            );
        }
    ...
    public static function getBaloonHtml($id=false, $hideBtn=false, $template='.default'){
        ...
        if(!$id){
            $result->addError(new Error(Loc::getMessage('AWZ_BELPOST_HELPER_ERR_EMPTY_ID')));
            return $result;
        }

        $resPoint = PvzTable::getPvz($id);
        if(!$resPoint){
            $result->addError(new Error(Loc::getMessage('AWZ_BELPOST_HELPER_ERR_EMPTY_DATA')));
            return $result;
        }
        ...
        ob_start();
        $APPLICATION->IncludeComponent("awz:belpost.baloon",	$template,
            Array(
                "DATA" => $item,
    ...
    public static function getDefTarifs(string $type = 'fl'): array
    {
        static $tarif_cache;

        if(!$tarif_cache) $tarif_cache = [];

        if(!isset($tarif_cache[$type])){
            $tarifs = [];

            try{
                $tarifs = unserialize(
                    Option::get("awz.belpost", "belpost_tarifs_".$type, "", ""),
            ...
            $tarif_cache[$type] = $tarifs;
        }

        return $tarif_cache[$type];
    }
